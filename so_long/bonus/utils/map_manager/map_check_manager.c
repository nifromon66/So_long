...
/*   Updated: 2025/01/22 06:49:25 by nifromon         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	map_check_manager(t_map **map)
{
	if (map_check_rectangle((*map)->map, (*map)->width, (*map)->height) == -1)
		error_manager("Map is not rectangle : irregular width", -1, map);
	if (map_check_closed((*map)->map, (*map)->width, (*map)->height) == -1)
		error_manager("Map isn't enclosed by walls", -1, map);
	if (map_check_char((*map)->map, (*map)->width, (*map)->height,
			&((*map)->count_c)) == -1)
		error_manager("Map has an invalid character", -1, map);
