...
/*   Updated: 2025/01/21 04:51:18 by nifromon         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	map_init(t_map **map_data)
{
	*map_data = (t_map *)malloc(sizeof(t_map));
	if (!*map_data)
		error_manager("Memory allocation error", -2, map_data);
...
void	map_manager(t_map **map_data)
{
	...
	if ((*map_data)->fd == -1)
		error_manager("Couldn't open map file", -2, map_data);
	map_read(map_data);
	map_init_directions(map_data);
	map_check_rectangle(map_data);
	map_check_enclosement(map_data);
	map_check_characters(map_data);
	(*map_data)->count_c = (*map_data)->cnt_c;
	map_check_elements(map_data);
	map_find_spawn(map_data);
	map_find_exit(map_data);
	start_x = (*map_data)->pos_p[0];
	start_y = (*map_data)->pos_p[1];
	if (map_check_way_exit(start_x, start_y, map_data) == 0)
		error_manager("No valid way to the exit", -2, map_data);
	free_visited(map_data);
	map_init_directions(map_data);
	if (map_check_way_collect(start_x, start_y, map_data) == 0)
		error_manager("No valid way to collectibles", -2, map_data);
}
...
void	map_valid_ext(t_map **map_data)
{
	...
	if (ft_strncmp(format, ".ber", 4) != 0)
		error_manager("Map has invalid file extension", -2, map_data);
}
...
void	map_read(t_map **map_data)
{
	...
	if ((*map_data)->map[i] == NULL)
		error_manager("Map is empty", -2, map_data);
	i++;
	(*map_data)->h = i;
	while (1)
	{
